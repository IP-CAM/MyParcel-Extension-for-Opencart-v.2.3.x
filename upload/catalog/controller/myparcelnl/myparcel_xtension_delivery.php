...
class ControllerMyparcelnlMyparcelXtensionDelivery extends Controller
{
    public $data = array();
    ...
    function index()
    {
        ...
        if(isset($myparcel_delivery[$shipping_method_code])){
            ...
            if($shipping_method_code == 'delivery'){
                ...
                $this->data['date']  = (isset($this->session->data['delivery_date']) && date('Y-m-d',strtotime($this->data['min_date'])) < date('Y-m-d',strtotime($this->session->data['delivery_date']))) ? date('Y-m-d',strtotime($this->session->data['delivery_date'])) : date('Y-m-d',strtotime($this->data['min_date']));
        ...
        $this->response->setOutput($this->load->view(MyParcel()->getMyparcelXtensionViewPath() . 'myparcel_xtension_delivery.tpl', $this->data));
...
//                    }
            ...
            if(isset($shipping_address['iso_code_2']) && $shipping_address['iso_code_2'] == 'NL' && $shipping_method_code == 'delivery'){
                ...
                /** @var MyParcel_Shipment_Checkout $checkout_helper **/
                $checkout_helper = $shipment_class->checkout;
                $additional_service = $checkout_helper->getDeliveryPrices(false, true, '', true, 0,0);
                $arr_additional_service = [];
                $array_additional_key = ['signed','only_recipient'];
                $default_price_0_text = isset($myparcelnl_checkout_settings['default_price_0_text']) ? $myparcelnl_checkout_settings['default_price_0_text'] : '';
                foreach ($additional_service[$shipping_address['iso_code_2']] as $key => $value){
        ...
        if($shipping_method_code != ''){
            $json['html'] = $this->load->view(MyParcel()->getMyparcelXtensionViewPath() . 'myparcel_xtension_'. $shipping_method_code .'_change.tpl', $this->data);
            $json['success'] = true;
        }
